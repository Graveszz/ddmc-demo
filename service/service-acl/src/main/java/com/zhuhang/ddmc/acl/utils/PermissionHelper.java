package com.zhuhang.ddmc.acl.utils;
...
public class PermissionHelper {
    ...
    private static Permission findChildren(Permission parent, List<Permission> allList) {
        parent.setChildren(new ArrayList<Permission>());
        for (Permission permission : allList) {
            if (parent.getId().longValue() == permission.getPid().longValue()) {
                if (parent.getChildren() == null) {
                    parent.setChildren(new ArrayList<>());
                }
                permission.setLevel(parent.getLevel() + 1);
                parent.getChildren().add(findChildren(permission, allList));
            }
        }
        return parent;
    }
